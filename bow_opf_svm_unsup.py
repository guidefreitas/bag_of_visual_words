...
class BoWOpfSvmUnsup:
  ...
  def opf_cluster(self, descriptors):
    opf_desc_filename = "desc_opf.txt"
    opf_desc_filename = os.path.realpath("desc_opf.txt")
  ...
  def train(self):
    ...
    for idx, image_info in enumerate(total_images_info):
      ...
      if (descriptors != None) and (len(descriptors) > 0):
        ...
        #self.log("self.result_classes: " + str(len(self.result_classes)))
        #self.log("self.result_data: " + str(idx))
